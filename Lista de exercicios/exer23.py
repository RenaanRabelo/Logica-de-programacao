#Para o enunciado a seguir foi elaborado um algoritmo em Português Estruturado que contém
#erros, identifique os erros no algoritmo apresentado abaixo:
#Tendo como dados de entrada o nome, a altura e o sexo (M ou F) de uma pessoa, calcule
#e mostre seu peso ideal, utilizando as seguintes fórmulas:
#- para sexo masculino: peso ideal = (72.7 * altura) - 58
#- para sexo feminino: peso ideal = (62.1 * altura) - 44.7
#inicio
#ler nome
#ler sexo
#se sexo = M então
#peso_ideal = (72.7 * altura) - 58
#senão
#peso_ideal = (62.1 * altura) – 44.7
#fim_se
#escrever peso_ideal
#fim

nome = str(input('Digite o nome da pessoa: '))
altura = float(input('Digite a altura da pessoa: '))
print('Digite m para sexo MASCULINO e f para FEMININO')
sexo = str(input('Digite a letra aqui: '))

while sexo != 'm' and sexo != 'f':
    print('Opção inválida, digite m para masculino ou f para feminino')
    sexo = input('Digite a letra aqui: ')
    
if sexo == 'm':
    pesoideal = (72.7 * altura) - 58
else:
    pesoideal = (62.1 * altura) - 44.7

print('O peso ideal é: {:.1f}kg'.format(pesoideal))